...
// Colors
const ASKLUKE_BLUE: [number, number, number] = [24, 95, 165];
...
function renderFooter(doc: jsPDF) {
  ...
  doc.text("AskLuke — Confidential Patient Information", MARGIN, FOOTER_Y);
...
export function generateIntakePdf(data: IntakeData): void {
  ...
  // ── Header bar ──
  doc.setFillColor(...ASKLUKE_BLUE);
  doc.rect(0, 0, PAGE_W, 72, "F");

  doc.setTextColor(255, 255, 255);
  doc.setFontSize(22);
  doc.setFont("times", "bold");
  doc.text("AskLuke", MARGIN, 32);
  ...
  doc.setTextColor(...MUTED_COLOR);
  const disclaimer =
    "This summary was generated by AskLuke AI intake and is not a medical diagnosis. " +
  ...
  doc.save("askluke-intake-summary.pdf");
}
